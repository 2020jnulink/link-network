...
func listContractTypes(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	input := struct {
		//`json:"shop_type"` 이런 식으로 태그를 달아두면 마샬링할 때 JSON형식에 ShopType이 아니라 shop_type으로 저장된다.
	...
	//부분 합성키로 원장을 조회한다. 반복자를 책갈피처럼 사용할 수 있다.
	...
	for resultsIterator.HasNext() {
		...
		// 익명 구조체를 만들땐 뒤에 {}를 한번 더 쓰고 내용물을 적는다.  여기선 아무 내용 적지 않을 꺼라 그냥 {}만 해준다.
		ct := struct {
